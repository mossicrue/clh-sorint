...
# variables
MANIFEST_PATH="https://raw.githubusercontent.com/mossicrue/clh-sorint/master/kubernetes/k8s-resources.yaml"
GAME_ROUTE_LINK=""
...
(${KUBECTL_COMMAND} apply -f ${MANIFEST_PATH} &>/dev/null)

# Wait for the Pod to be in running state
POD_STATUS="$(${KUBECTL_COMMAND} get pod -n clh-game --no-headers=true -o custom-columns=READY:.status.phase 2>/dev/null)"
...
GAME_ROUTE_LINK=$(${KUBECTL_COMMAND} get route clh-game -n clh-game --output=custom-columns=HOST:.spec.host --no-headers=true 2>/dev/null)
